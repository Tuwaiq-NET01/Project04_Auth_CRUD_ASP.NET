using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using System.Security.Claims;
using System.Threading.Tasks;
using Amazon;
using Amazon.S3;
using Amazon.S3.Transfer;
using Aspose.Pdf;
using eBookshelf.Data;
using eBookshelf.Models;
using EpubSharp;
using Microsoft.AspNetCore.Hosting;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Extensions.Configuration;
using pdfcrowd;
using RestSharp;

// For more information on enabling MVC for empty projects, visit https://go.microsoft.com/fwlink/?LinkID=397860

namespace eBookshelf.Controllers
{
    public class ConverterController : Controller
    {
        private IHostingEnvironment Environment;
        private readonly ApplicationDbContext _context;
        private readonly IConfiguration _config;

        public ConverterController(IHostingEnvironment _environment,ApplicationDbContext context, IConfiguration config)
        {
            Environment = _environment;
            _context = context;
            _config = config;
        }

        // GET: /<controller>/

        public ActionResult Index()
        {

            return View();
        }

        [HttpPost]
        public async Task<ActionResult> Index([Bind("Name,PageUrl")] ConverterModel model)
        {
            if (ModelState.IsValid)
            {
                EbookViewModel ebookViewModel = null;

                try
                {
                    var result = await convert(model.PageUrl);
                   //if(result.GetType().Equals("VirtualFileResult") )

                    var stream = new MemoryStream(System.IO.File.ReadAllBytes(Environment.WebRootPath+"\\Temp\\output.epub"));
                    var fileName = Guid.NewGuid().ToString() + ".epub";

                    var formFile = new FormFile(stream, 0, stream.Length, "streamFile", fileName);

                    ebookViewModel = new EbookViewModel
                    {
                        Name = model.Name,
                        Content = formFile
                    };
                    
                }
                catch (Exception e)
                {
                    //ms.Dispose();
                    throw;
                }
                //finally
                //{
                //    ms.Dispose();
                //}

                try
                {
                    if (ebookViewModel.Content != null)
                    {
                        if (ebookViewModel.Content.Length > 0)
                        {

                            var ebookModel = new EbookModel()
                            {
                                OwnerId = User.FindFirstValue(ClaimTypes.NameIdentifier),
                                CreationDate = DateTime.Now,
                                Name = ebookViewModel.Name,
                                //Author = ebookViewModel.Author,
                                //CoverImg = ebookViewModel.CoverImg
                            };

                            await using (var newMemoryStream = new MemoryStream())
                            {
                                ebookViewModel.Content.CopyTo(newMemoryStream);

                                EpubBook epub = EpubReader.Read(newMemoryStream, false);
                                //if (epub.CoverImage != null)
                                //{
                                var defaultCover = "";
                                ebookModel.CoverImg = string.Format("data:image/png;base64,{0}", (epub.CoverImage != null) ? Convert.ToBase64String(epub.CoverImage) : defaultCover);
                                //}
                                //else { ebookModel.CoverImg = ""; }
                                if (epub.Authors.ToArray().Length > 0)
                                {
                                    ebookModel.Author = String.Join(',', epub.Authors.ToArray());
                                }
                                else { ebookModel.Author = ""; }

                            }

                            await UploadFileToS3(ebookViewModel.Content);

                            ebookModel.ContentUrl = "https://ebookshelf0027.s3.eu-central-1.amazonaws.com/" + ebookViewModel.Content.FileName;
                            _context.Add(ebookModel);
                            await _context.SaveChangesAsync();
                            return RedirectToAction("Index", "EBook");

                        }
                    }
                }
                catch (Exception ex)
                {
                    return View("Error", new ErrorViewModel() { RequestId = ex.Message + "\n" + ex.InnerException });
                }
            }
            return View() ;
        }
        public async Task<IActionResult> convert(string url)
        {
            if (url == null) return NotFound();

            string filePath = Environment.WebRootPath + "\\Temp\\file.pdf";

            try
            {
                pdfcrowd.HtmlToPdfClient client =
                 new pdfcrowd.HtmlToPdfClient("ThamerMashni2", "7b83070c5346ea6aa7a22fca796c8fad");

                if (System.IO.File.Exists(filePath))
                {
                    System.IO.File.Delete(filePath);
                }

                FileStream fileStream = System.IO.File.Create(filePath);
                await using (fileStream)
                {
                    try
                    {
                        client.convertUrlToStream(url, fileStream);
                    }
                    catch (Error)
                    {
                        fileStream.Close();
                        System.IO.File.Delete(filePath);
                        throw;
                    }
                }

            }
            catch (pdfcrowd.Error why)
            {
                // report the error
                System.Console.Error.WriteLine("Pdfcrowd Error: " + why);

                throw;
            }
            
            EpubSaveOptions options = new EpubSaveOptions();
            options.ContentRecognitionMode = EpubSaveOptions.RecognitionMode.Flow;

            //var stream = new MemoryStream();
            using (var pdfDocument = new Aspose.Pdf.Document(filePath))
            {
                Console.WriteLine($"Pages {pdfDocument.Pages.Count}");
                pdfDocument.Save(Environment.WebRootPath + "\\Temp\\output.epub", options);
                //pdfDocument.Save(stream, options);

            }
            
            var fileName = Guid.NewGuid().ToString() + ".epub";

            //return new FileStreamResult(stream, "application/epub+zip")
            //{
            //    FileDownloadName = fileName
            //};

            return File("\\Temp\\output.epub", "application/epub+zip",fileName);

        }

        public async Task UploadFileToS3(IFormFile file)
        {
            using (var client = new AmazonS3Client(_config["aws:s3AccessKey"], _config["aws:s3SecretAccessKey"], RegionEndpoint.EUCentral1))
            {
                using (var newMemoryStream = new MemoryStream())
                {
                    file.CopyTo(newMemoryStream);

                    var uploadRequest = new TransferUtilityUploadRequest
                    {
                        InputStream = newMemoryStream,
                        Key = file.FileName,
                        BucketName = "ebookshelf0027",
                        CannedACL = S3CannedACL.PublicRead
                    };

                    var fileTransferUtility = new TransferUtility(client);
                    await fileTransferUtility.UploadAsync(uploadRequest);
                }
            }
        }

    }
}
